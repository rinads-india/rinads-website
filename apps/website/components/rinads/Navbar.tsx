    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[45] flex flex-col items-center justify-center gap-10 overflow-y-auto bg-rinads-primary-darkest px-6 pb-28 pt-24"
        >
    ...
    <>
      <DynamicIslandNav expanded={open} ariaLabel="Site">
        ...
        <div className="hidden min-w-0 flex-1 items-center justify-center gap-6 lg:flex xl:gap-8">
          {LINKS.map((link) =>
            link.href.startsWith("/") && !link.href.includes("#") ? (
              <Link key={link.href} href={link.href} className={islandLinkClass}>
                {link.label}
              </Link>
            ) : (
